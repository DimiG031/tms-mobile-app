...
**✅ RAZREŠENO — „404 na detalj listića" NIJE bio bug (backend, 2026-07-08):** bio je samo **lokalni dev server (Turbopack)** koji nije registrovao dinamičku rutu `payslips/[id]` dok se ne rekompajlira. **Na produkciji vraća 200 sa punim stavkama.** Seed je na produ i verifikovan uživo. Na preview APK-u (gađa prod) listić sa stavkama radi. Mobilni bez izmene. Takođe: backend dodao **server-PDF** (`GET /me/payslips/:id/pdf`, `/per-diem/:id/pdf`, srpska slova) — opcija umesto našeg klijentskog `expo-print` PDF-a; korišćenje traži `expo-file-system` (nov build), pa zasad ostaje naš custom PDF (moćan, zaglavlje vozača, rate 6/18).

Istorijski zapis (prvobitna sumnja na bug):
**🐞 (bilo prijavljeno) — detalj listića vraća 404 (2026-07-08):** lista `GET /api/mobile/me/payslips?year=2026` **radi** (vraća `{ id: "cmrbh82go001bunnsdjsca46j", net: 67000, status: "CONFIRMED", ... }`), ali `GET /api/mobile/me/payslips/cmrbh82go001bunnsdjsca46j` vraća **404** za taj isti id. Dnevnice (`/per-diem`) rade 200. Verovatno pogrešan lookup/scoping u detail ruti (`src/app/api/mobile/me/payslips/[id]/route.ts`) — npr. traži po `employeeId`/`companyId` uslovu koji lista ne primenjuje, ili po pogrešnom ključu. Mobilni šalje tačno id iz liste. **Molba: uskladiti detail lookup sa listom (isti scope/ključ).** Na mobilnoj se vidi „Greška pri učitavanju stavki".
